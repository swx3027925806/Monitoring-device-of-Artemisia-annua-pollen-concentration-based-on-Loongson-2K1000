...
char *GetFilename(char *p)
{
	static char name[20] = { "" };
	char *q = strrchr(p, '/') + 1;
	strncpy(name, q, 3);//后面的6是图片名长度（不包括后缀），根据自己的需要进行修改
	return name;
}
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
	...
	while (1) {
		if (filename) {
			...
			int m = plist->size;
			if (access("/home/loongson/darknet/data/out", 0) == -1)//"/home/.../darknet/data"根据自己的路径修改
			{
				if (mkdir("/home/loongson/darknet/data/out", 0777))//"/home/.../darknet/data"根据自己的路径修改
				{
			...
			for (i = 0; i < m; ++i) {
				...
				if (outfile) {
					...
					char b[2048];
					sprintf(b, "/home/loongson/darknet/data/out/%s", GetFilename(path));//"/home/.../darknet/data"根据自己的路径修改
